const ALL_ROOMS_AND_SPACES = [
  {
    title: "Habitacion Arrayan",
    description: "Habitación matrimonial en planta baja con baño privado",
    images: [
      {
        src: "/HabitacionArrayan/habitacion-arrayan-1.jpg",
        alt: "Habitacion Arrayan 1"
      },
      {
        src: "/HabitacionArrayan/habitacion-arrayan-2.jpg",
        alt: "Habitacion Arrayan 2"
      },
      {
        src: "/HabitacionArrayan/habitacion-arrayan-3.jpg",
        alt: "Habitacion Arrayan 3"
      },
      {
        src: "/HabitacionArrayan/habitacion-arrayan-bano-1.jpg",
        alt: "Habitacion Arrayan bano"
      },
      {
        src: "/HabitacionArrayan/habitacion-arrayan-bano-2.jpg",
        alt: "Habitacion Arrayan bano"
      },
      {
        src: "/HabitacionArrayan/habitacion-arrayan-escalera.jpg",
        alt: "Habitacion Arrayan escalera"
      },
      {
        src: "/HabitacionArrayan/habitacion-arrayan-escalera-2.jpg",
        alt: "Habitacion Arrayan escalera"
      },
      {
        src: "/HabitacionArrayan/habitacion-arrayan-vista.jpg",
        alt: "Habitacion Arrayan vista"
      }
    ],
    amenities: [
      {
        icon: "bi bi-tv-fill",
        description: "Smart TV"
      },
      {
        icon: "bi bi-droplet-fill",
        description: "habitacion privada con baño Privado"
      },
      {
        icon: "bi bi-snow2",
        description: "Frigobar"
      },
      {
        icon: "bi bi-people-fill",
        description: "Con desayunador compartido"
      }
    ]

  },
  {
    title: "Habitacion Sol",
    description: "Habitación matrimonial en planta baja con baño privado",
    images: [
      {
        src: "/HabitacionSol/habitacion-sol-1.jpg",
        alt: "Habitacion Sol"
      },
      {
        src: "/HabitacionSol/habitacion-sol-2.jpg",
        alt: "Habitacion Sol"
      },
      {
        src: "/HabitacionSol/habitacion-sol-3.jpg",
        alt: "Habitacion Sol"
      },
      {
        src: "/HabitacionSol/habitacion-sol-bano.jpg",
        alt: "Habitacion Sol bano"
      },
      {
        src: "/HabitacionSol/habitacion-sol-bano-2.jpg",
        alt: "Habitacion Sol bano"
      }
    ],
    amenities: [
      {
        icon: "bi bi-tv-fill",
        description: "Smart TV"
      },
      {
        icon: "bi bi-droplet-fill",
        description: "Habitacion privada con baño Privado"
      },
      {
        icon: "bi bi-snow2",
        description: "Frigobar"
      },
      {
        icon: "bi bi-fork-knife",
        description: "Desayunador Completo"
      }
    ]
  },
  {
    title: "Habitacion Tierra",
    description: "Habitación matrimonial en planta baja con baño privado",
    images: [
      {
        src: "/HabitacionTierra/habitacion-tierra-1.jpg",
        alt: "Habitacion Tierra"
      },
      {
        src: "/HabitacionTierra/habitacion-tierra-2.jpg",
        alt: "Habitacion Tierra"
      },
      {
        src: "/HabitacionTierra/habitacion-tierra-3.jpg",
        alt: "Habitacion Tierra"
      },
      {
        src: "/HabitacionTierra/habitacion-tierra-4.jpg",
        alt: "Habitacion Tierra"
      },
      {
        src: "/HabitacionTierra/habitacion-tierra-bano-1.jpg",
        alt: "Habitacion Tierra bano"
      },
      {
        src: "/HabitacionTierra/habitacion-tierra-bano-2.jpg",
        alt: "Habitacion Tierra bano"
      }
    ],
    amenities: [
      {
        icon: "bi bi-tv-fill",
        description: "Smart TV"
      },
      {
        icon: "bi bi-droplet-fill",
        description: "Habitacion privada con baño Privado"
      },
      {
        icon: "bi bi-snow2",
        description: "Frigobar"
      },
      {
        icon: "bi bi-fork-knife",
        description: "Desayunador Completo"
      }
    ]
  },
  {
    title: "Habitacion Cipres",
    description: "Habitación matrimonial en primer piso con baño compartido",
    images: [
      {
        src: "/HabitacionCipres/habitacion-cipres-1.jpg",
        alt: "Habitacion Cipres"
      },
      {
        src: "/HabitacionCipres/habitacion-cipres-2.jpg",
        alt: "Habitacion Cipres"
      },
      {
        src: "/HabitacionCipres/habitacion-cipres-bano-1.jpg",
        alt: "Habitacion Cipres bano"
      }
    ],
    amenities: [
      {
        icon: "bi bi-droplet-fill",
        description: "Elección de Baño Privado"
      },
      {
        icon: "bi bi-door-closed-fill",
        description: "Habitacion privada"
      },
      {
        icon: "bi bi-people-fill",
        description: "Con desayunador compartido"
      },
    ]
  },
  {
    title: "Habitacion Luz",
    description: "Habitación matrimonial en primer piso con baño compartido",
    images: [
      {
        src: "/HabitacionLuz/habitacion-luz-1.jpg",
        alt: "Habitacion Luz"
      },
      {
        src: "/HabitacionLuz/habitacion-luz-2.jpg",
        alt: "Habitacion Luz"
      },
      {
        src: "/HabitacionLuz/habitacion-luz-bano.jpg",
        alt: "Habitacion Luz bano"
      }
    ],
    amenities: [
      {
        icon: "bi bi-droplet-fill",
        description: "Elección de Baño Privado"
      },
      {
        icon: "bi bi-door-closed-fill",
        description: "Habitacion privada"
      },
      {
        icon: "bi bi-people-fill",
        description: "Con desayunador compartido"
      },
    ]
  },
  {
    title: "Espacio Compartido",
    description: "Espacio en común de las habitaciones (Arrayan, Cipres y Luz) en el primer piso",
    images: [
      {
        src: "/EspacioCompartido/espacio-compartido-living.jpg",
        alt: "Espacio Compartido Living"},
      {
        src: "/EspacioCompartido/espacio-compartido-living-2.jpg",
        alt: "Espacio Compartido Living"
      },
      {
        src: "/EspacioCompartido/espacio-compartido-living-3.jpg",
        alt: "Espacio Compartido Living"
      },
      {
        src: "/EspacioCompartido/espacio-compartido-living-4.jpg",
        alt: "Espacio Compartido Living"
      },
      {
        src: "/EspacioCompartido/espacio-compartido-living-5.jpg",
        alt: "Espacio Compartido Living"
      },
      {
        src: "/EspacioCompartido/espacio-compartido-cocina.jpg",
        alt: "Espacio Compartido Cocina"
      },
      {
        src: "/EspacioCompartido/espacio-compartido-entrada.jpg",
        alt: "Espacio Compartido Entrada"
      },
      {
        src: "/EspacioCompartido/espacio-compartido-escalera.jpg",
        alt: "Espacio Compartido Escalera"
      }
  
    ],
    amenities: [
      {
        icon: "bi bi-tv-fill",
        description: "Smart TV"
      },
      {
        icon: "bi bi-cup-hot-fill",
        description: "Desayunador completo"
      },
      {
        icon: "bi bi-wifi",
        description: "Wi-Fi"
      }
    ]
  },
  {
    title: "Exterior",
    description: "Exterior de las habitaciones y espacios comunes",
    images: [
      {
        src: "/Exterior/exterior-3.jpg",
        alt: "Exterior"
      },
      {
        src: "/Exterior/exterior.jpg",
        alt: "Exterior"
      },
      {
        src: "/Exterior/exterior-2.jpg",
        alt: "Exterior"
      },
    ],
    amenities: [
      {
        icon: "bi bi-tree-fill",
        description: "Terreno Cercado"
      },
      {
        icon: "bi bi-car-front-fill",
        description: "Estacionamiento Privado"
      },
      {
        icon: "bi bi-fire",
        description: "Quincho con Parrilla"
      }
    ]
  }
]

export default ALL_ROOMS_AND_SPACES